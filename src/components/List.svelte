<script lang="ts">
	...
	function deleteReservation(deleteId: number) {
		handleReservationData.update((value) => {
			return value.filter((table) => table.id !== deleteId);
		});
	}

	function completedReservation(deleteId: number) {
		reservedData = reservedData.filter((table) => table.id !== deleteId);
	}

	function moveEditPage(editTableNumber: number) {
		goto(`/editReservation/${editTableNumber}`);
	}
</script>
...
<style>
	...
	.itemContainer:hover,
	.itemContainer:hover .phoneContainer,
	.itemContainer:hover .deleteBtn {
